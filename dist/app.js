...
const handleDrag = (e) => {
    ...
    rightBox.addEventListener("drop", (_) => {
        if (selected != null) {
            selected.classList.add("on-right"); // change style when the box is in the right container
    ...
    leftBox.addEventListener("drop", (_) => {
        if (selected != null) {
            selected.classList.remove("on-right");
            addBox.insertAdjacentElement("beforebegin", selected);
            selected = null;
        }
        leftBox.classList.remove("droppable");
        rightBox.classList.remove("droppable");
